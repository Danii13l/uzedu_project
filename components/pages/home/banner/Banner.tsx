...
export const Banner: FC<{ data: BannerInt }> = ({ data }): JSX.Element => {
    return <div className={s.banner} style={{ backgroundImage: `url(${process.env.NEXT_PUBLIC_BASE_URL}${data?.url})` }}>
        <SectionWrapper>
            <div className={s.inner}>
                ...
                <div className={s.content}>
                    ...
                    <blockquote className={s.blockquote}>{data?.description}</blockquote>
                </div>

                <div className={s.line_content}>
                    <div className={s.line_items}>
                        <h4 className={s.l_i_title}>{data?.pupils}</h4>
                        <p className={s.l_i_text}>Учеников</p>
                    </div>
                    <div className={s.line_items}>
                        <h4 className={s.l_i_title}>{data?.teachers}</h4>
                        <p className={s.l_i_text}>Преподователей</p>
                    </div>
                    <div className={s.line_items}>
                        <h4 className={s.l_i_title}>{data?.barkamol}</h4>
                        <p className={s.l_i_text}>“Школ Баркамол авлод”</p>
                    </div>
                    <div className={s.line_items}>
                        <h4 className={s.l_i_title}>{data?.houses}</h4>
